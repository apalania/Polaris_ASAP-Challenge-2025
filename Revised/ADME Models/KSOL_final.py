...
# XGBoost Regression Pipeline - KSOL ONLY VERSION
...
# --- KSOL SPECIFIC CONFIGURATION ---
# Fixed file paths for KSOL endpoint
KSOL_POLARIS_FILE = 'KSOL_POLARIS.csv'
KSOL_AUGMENTED_FILE = 'KSOL_MERGED.csv'
KSOL_TEST_FILE = 'polaris-test.csv'
KSOL_TARGET_COLUMN = 'KSOL'

# KSOL requires log transformation and no negative values
KSOL_NEEDS_LOG_TRANSFORM = True
KSOL_ALLOW_NEGATIVE = False
...
def load_and_prepare_data(descriptor_df, target_file):
    """Load and merge descriptor and target data with KSOL-specific preprocessing."""
    
    target_data_full = pd.read_csv(target_file)
    print(f"Available columns in {target_file}: {target_data_full.columns.tolist()}")
    
    target_data = target_data_full[["SMILES", KSOL_TARGET_COLUMN]]
    data = pd.merge(descriptor_df, target_data, on="SMILES", how="inner")
    data.dropna(inplace=True)
    data = data.loc[:, ~data.columns.str.contains('^Unnamed')]
    data[KSOL_TARGET_COLUMN] = pd.to_numeric(data[KSOL_TARGET_COLUMN], errors='coerce')
    
    
    # KSOL-specific preprocessing: Log10(y_clipped + 1)
    if KSOL_NEEDS_LOG_TRANSFORM:
        if not KSOL_ALLOW_NEGATIVE:
            # Clip negative values to 0 instead of dropping them
            data[KSOL_TARGET_COLUMN] = data[KSOL_TARGET_COLUMN].clip(lower=0)
        
        # Apply Log10(y + 1)
        data[KSOL_TARGET_COLUMN] = np.log10(data[KSOL_TARGET_COLUMN] + 1)
    else:
        data = data[~data[KSOL_TARGET_COLUMN].isna()]
   -
...
def evaluate_model(y_true, y_pred):
    ...
    # Convert back from Log10 space: y = (10^x) - 1
    if KSOL_NEEDS_LOG_TRANSFORM:
        y_true_original = np.power(10, y_true) - 1
        y_pred_original = np.power(10, y_pred) - 1
    else:
...
def run_experiment(data, split_strategy, n_splits=10, output_dir=None, config_name=None):
    ...
    X = data.drop(columns=["SMILES", KSOL_TARGET_COLUMN])
    y = data[KSOL_TARGET_COLUMN]
...
def ensemble_predict_on_test(split_models, test_file, descriptor_file, config_name, output_dir, X_full_columns):
    ...
    # KSOL-specific inverse transformation: (10^x) - 1
    if KSOL_NEEDS_LOG_TRANSFORM:
        ensemble_predictions = np.power(10, ensemble_predictions_transformed) - 1
        if not KSOL_ALLOW_NEGATIVE:
    ...
    results_df = pd.DataFrame({
        ...
        f"Predicted_{KSOL_TARGET_COLUMN}": ensemble_predictions
    })
    ...
    for i, preds_transformed in enumerate(all_predictions):
        ...
        if KSOL_NEEDS_LOG_TRANSFORM:
            preds = np.power(10, preds_transformed) - 1
            if not KSOL_ALLOW_NEGATIVE:
        ...
        individual_df = pd.DataFrame({
            "SMILES": test_descriptors["SMILES"],
            f"Predicted_{KSOL_TARGET_COLUMN}_split_{i}": preds
        })
...
if __name__ == "__main__":
    ...
    print("\n" + "="*80)
    print("KSOL ENDPOINT PIPELINE - REPRODUCIBLE VERSION")
    print("="*80)
    ...
    # Create output directory for KSOL
    output_dir = "results_KSOL"
    ensure_directory(output_dir)
    
    print(f"\n{'#'*80}")
    print(f"STARTING PIPELINE FOR KSOL ENDPOINT")
    ...
    for data_name, data_file in data_options.items():
        for feature_name, feature_file in FEATURE_OPTIONS.items():
            for split_strategy in SPLIT_OPTIONS:
                ...
                config_name = f"KSOL_{data_name}_{feature_name}_{split_strategy}"
                ...
                try:
                    ...
                    X_full = full_data.drop(columns=["SMILES", KSOL_TARGET_COLUMN])
                    
                    # Make ensemble predictions
                    ensemble_predict_on_test(
                        split_models=split_models,
                        test_file=KSOL_TEST_FILE,
    ...
    print("\n\n" + "="*80)
    print("ALL EXPERIMENTS COMPLETED FOR KSOL ENDPOINT!")
    print("="*80)
    checkpoint.print_summary()
    ...
    with open(os.path.join(checkpoint.checkpoint_dir, "final_report_KSOL.json"), 'w') as f:
        json.dump(final_report, f, indent=4)
    
    print("\nFinal report saved to checkpoints/final_report_KSOL.json")
