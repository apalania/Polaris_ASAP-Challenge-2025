...
# --- MDR1 SPECIFIC CONFIGURATION ---
# Fixed file paths for MDR1 endpoint
MDR1_POLARIS_FILE = 'MDR1_POLARIS.csv'
MDR1_AUGMENTED_FILE = 'MDR1_MERGED.csv'
MDR1_TEST_FILE = 'polaris-test.csv'
MDR1_TARGET_COLUMN = 'MDR1-MDCK2'

# MDR1 requires log transformation and no negative values
MDR1_NEEDS_LOG_TRANSFORM = True
MDR1_ALLOW_NEGATIVE = False
...
def load_and_prepare_data(descriptor_df, target_file):
    """Load and merge descriptor and target data with MDR1-specific preprocessing."""
    
    target_data_full = pd.read_csv(target_file)
    print(f"Available columns in {target_file}: {target_data_full.columns.tolist()}")
    
    target_data = target_data_full[["SMILES", MDR1_TARGET_COLUMN]]
    data = pd.merge(descriptor_df, target_data, on="SMILES", how="inner")
    data.dropna(inplace=True)
    data = data.loc[:, ~data.columns.str.contains('^Unnamed')]
    data[MDR1_TARGET_COLUMN] = pd.to_numeric(data[MDR1_TARGET_COLUMN], errors='coerce')
    
    # MDR1-specific preprocessing
    if MDR1_NEEDS_LOG_TRANSFORM:
        if not MDR1_ALLOW_NEGATIVE:
            data = data[data[MDR1_TARGET_COLUMN] > 0]
        data[MDR1_TARGET_COLUMN] = np.log1p(data[MDR1_TARGET_COLUMN])
    else:
        data = data[~data[MDR1_TARGET_COLUMN].isna()]
...
def evaluate_model(y_true, y_pred):
    ...
    # Convert back from log space for MDR1
    if MDR1_NEEDS_LOG_TRANSFORM:
...
def run_experiment(data, split_strategy, n_splits=10, output_dir=None, config_name=None):
    ...
    X = data.drop(columns=["SMILES", MDR1_TARGET_COLUMN])
    y = data[MDR1_TARGET_COLUMN]
...
def ensemble_predict_on_test(split_models, test_file, descriptor_file, config_name, output_dir, X_full_columns):
    ...
    # MDR1-specific inverse transformation and clipping
    if MDR1_NEEDS_LOG_TRANSFORM:
    ...
    results_df = pd.DataFrame({
        ...
        f"Predicted_{MDR1_TARGET_COLUMN}": ensemble_predictions
    })
    ...
    for i, preds_transformed in enumerate(all_predictions):
        ...
        individual_df = pd.DataFrame({
            "SMILES": test_descriptors["SMILES"],
            f"Predicted_{MDR1_TARGET_COLUMN}_split_{i}": preds
        })
...
if __name__ == "__main__":
    ...
    print("\n" + "="*80)
    print("MDR1 ENDPOINT PIPELINE - REPRODUCIBLE VERSION")
    print("="*80)
    ...
    # Create output directory for MDR1
    output_dir = "results_MDR1"
    ensure_directory(output_dir)
    
    print(f"\n{'#'*80}")
    print(f"STARTING PIPELINE FOR MDR1 ENDPOINT")
    ...
    for data_name, data_file in data_options.items():
        for feature_name, feature_file in FEATURE_OPTIONS.items():
            for split_strategy in SPLIT_OPTIONS:
                ...
                config_name = f"MDR1_{data_name}_{feature_name}_{split_strategy}"
                ...
                try:
                    ...
                    X_full = full_data.drop(columns=["SMILES", MDR1M_TARGET_COLUMN])
                    
                    # Make ensemble predictions
                    ensemble_predict_on_test(
                        split_models=split_models,
                        test_file=MDR1_TEST_FILE,
    ...
    print("\n\n" + "="*80)
    print("ALL EXPERIMENTS COMPLETED FOR MDR1 ENDPOINT!")
    print("="*80)
    checkpoint.print_summary()
    ...
    with open(os.path.join(checkpoint.checkpoint_dir, "final_report_MDR1.json"), 'w') as f:
        json.dump(final_report, f, indent=4)
    
    print("\nFinal report saved to checkpoints/final_report_MDR1.json")
