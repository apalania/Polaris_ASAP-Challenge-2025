...
def plot_model_performance(final_model, y_test, y_pred, best_params, mae, r2):
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/MDR1_feature_importance.png", dpi=600, format='png')
    plt.savefig(f"{output_dir}/MDR1_feature_importance.tif", dpi=600, format='tiff')
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/MDR1_actual_vs_predicted.png", dpi=600, format='png')
    plt.savefig(f"{output_dir}/MDR1_actual_vs_predicted.tif", dpi=600, format='tiff')
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/MDR1_residuals_density.png", dpi=600, format='png')
    plt.savefig(f"{output_dir}/MDR1_residuals_density.tif", dpi=600, format='tiff')
    plt.show()
...
def save_model_and_info(final_model, mae, r2, adjusted_r2, best_params):
    joblib.dump(final_model, "final_model_MDR1-MDCK2_mae.joblib")

    with open("model_metrics_MDR1-MDCK2_mae.txt", 'w') as f:
        f.write(f"Final Model Performance:\nMAE: {mae:.4f}\nR2 Score: {r2:.4f}\nAdjusted R2 Score: {adjusted_r2:.4f}\n")

    config = {
        "model_path": "final_model_MDR1-MDCK2_mae.joblib",
        "metrics_path": "model_metrics_MDR1-MDCK2_mae.txt",
        "best_params": best_params,
        "descriptor_file": "descriptors.txt",
        "descriptor_csv": "computed_descriptors_MDR1-MDCK2_mae.csv",
        "prediction_file": "polaris_predictions_MDR1-MDCK2_mae.csv"
    }

    with open("model_config_MDR1-MDCK2_mae.json", 'w') as f:
        json.dump(config, f, indent=4)

def predict_on_polaris_test(final_model, polaris_file, descriptor_file):
    polaris_descriptors = compute_and_save_descriptors(polaris_file, "polaris_computed_MDR1-MDCK2_descriptors_mae.csv", descriptor_file)
    X_polaris = polaris_descriptors.drop(columns=["SMILES"])
    predictions = np.expm1(final_model.predict(X_polaris))

    results = pd.DataFrame({"SMILES": polaris_descriptors["SMILES"], "Predicted_MDR1-MDCK2": predictions})
    results.to_csv("polaris_predictions_MDR1-MDCK2_mae.csv", index=False)

# Main Execution
if __name__ == "__main__":
    descriptor_output = "computed_descriptors_MDR1-MDCK2_mae.csv"
    MDR1_MDCK2_file = "MDR1_MDCK2_MERGED.csv"
    descriptor_list_file = "descriptors.txt"
    polaris_file = "polaris-test.csv"
    target_column = "MDR1-MDCK2"

    compute_and_save_descriptors(MDR1_MDCK2_file, descriptor_output, descriptor_list_file)
    data = load_and_prepare_data(descriptor_output, MDR1_MDCK2_file, target_column)
