...
def plot_model_performance(final_model, y_test, y_pred, best_params, mae, r2):
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/MLM_feature_importance.png", dpi=600, format='png')
    plt.savefig(f"{output_dir}/MLM_feature_importance.tif", dpi=600, format='tiff')
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/MLM_actual_vs_predicted.png", dpi=600, format='png')
    plt.savefig(f"{output_dir}/MLM_actual_vs_predicted.tif", dpi=600, format='tiff')
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{output_dir}/MLM_residuals_density.png", dpi=600, format='png')
    plt.savefig(f"{output_dir}/MLM_residuals_density.tif", dpi=600, format='tiff')
    plt.show()
...
def save_model_and_info(final_model, mae, r2, adjusted_r2, best_params):
    joblib.dump(final_model, "final_model_MLM_mae.joblib")

    with open("model_metrics_MLM_mae.txt", 'w') as f:
        f.write(f"Final Model Performance:\nMAE: {mae:.4f}\nR2 Score: {r2:.4f}\nAdjusted R2 Score: {adjusted_r2:.4f}\n")

    config = {
        "model_path": "final_model_MLM_mae.joblib",
        "metrics_path": "model_metrics_MLM_mae.txt",
        "best_params": best_params,
        "descriptor_file": "descriptors.txt",
        "descriptor_csv": "HLM_computed_descriptors_MLM_mae.csv",
        "prediction_file": "polaris_predictions_HLM_MLM_mae.csv"
    }

    with open("model_config_MLM_mae.json", 'w') as f:
        json.dump(config, f, indent=4)

def predict_on_polaris_test(final_model, polaris_file, descriptor_file):
    polaris_descriptors = compute_and_save_descriptors(polaris_file, "polaris_computed_MLM_descriptors_mae.csv", descriptor_file)
    X_polaris = polaris_descriptors.drop(columns=["SMILES"])
    predictions = np.expm1(final_model.predict(X_polaris))

    results = pd.DataFrame({"SMILES": polaris_descriptors["SMILES"], "Predicted_MLM": predictions})
    results.to_csv("polaris_predictions_MLM_mae.csv", index=False)


# Main Execution
if __name__ == "__main__":
    descriptor_output = "MLM_computed_descriptors_eval_mae.csv"
    hlm_file = "MLM_MERGED.csv"
