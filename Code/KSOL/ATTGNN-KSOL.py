...
def load_data(file_path, smiles_column="SMILES", target_column="KSOL"):
...
def train_model(data, epochs=200, batch_size=64, lr=3e-4, patience=15, weight_decay=1e-5, grad_accum_steps=2, bucket_name=None):
    ...
    for epoch in range(epochs):
        ...
        if r2 > best_r2 or (r2 == best_r2 and mae < best_mae):
            ...
            best_mae = mae
            early_stop_counter = 0
            torch.save(model.state_dict(), "best_model_val_ksolphase.pth")
            if bucket_name:
                upload_to_gcs(bucket_name, "best_model_val_phase.pth", "model1/best_model_val_ksolphase.pth")
        else:
    ...
    if bucket_name:
        upload_to_gcs(bucket_name, "validation_metrics.csv", "metric/validation_metrics.csv")
    ...
    for epoch in range(epochs // 2):
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            early_stop_counter = 0
            torch.save(model.state_dict(), "best_model_ksolfull.pth")
            if bucket_name:
                upload_to_gcs(bucket_name, "best_model_ksolfull.pth", "model1/best_model_ksolfull.pth")
        else:
            early_stop_counter += 1
            if early_stop_counter >= patience:
    ...
    if bucket_name:
        upload_to_gcs(bucket_name, "full_training_metrics.csv", "metric/full_training_metrics.csv")
...
if __name__ == '__main__':
    ...
    file_path = "gs://mounikasri/KSOL_MERGED.csv"
    ...
    plt.show()
    if GCS_BUCKET_NAME:
        upload_to_gcs(GCS_BUCKET_NAME, "residual_histogram.png", "GRAPHS/residual_histogram.png")
    ...
    plt.show()
    if GCS_BUCKET_NAME:
        upload_to_gcs(GCS_BUCKET_NAME, "actual_vs_predicted.png", "GRAPHS/actual_vs_predicted.png")
